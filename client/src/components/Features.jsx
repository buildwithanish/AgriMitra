        ...
        {highlightedFeatures.map((feature, index) => {
          ...
          return (
            <motion.button
              key={feature.title}
              type="button"
              onClick={openFeatureHub}
              initial={{ opacity: 0, y: 24 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true, amount: 0.2 }}
              transition={{ duration: 0.45, delay: index * 0.04 }}
              className="group rounded-[30px] border border-white/60 bg-white/75 p-6 text-left shadow-[0_24px_70px_rgba(7,23,15,0.1)] backdrop-blur-2xl transition duration-300 hover:-translate-y-1 hover:shadow-[0_35px_90px_rgba(7,23,15,0.14)] dark:border-white/10 dark:bg-white/5"
            >
